...
RUN set -ex \
    ...
    && pip install ffmpeg \
    && pip install apache-airflow[celery,postgres,jdbc,mysql,ssh${AIRFLOW_DEPS:+,}${AIRFLOW_DEPS}]==${AIRFLOW_VERSION} \
...
RUN mkdir -p ${AIRFLOW_USER_HOME}/dags \
    && mkdir -p ${AIRFLOW_USER_HOME}/webserver \
    && mkdir -p ${AIRFLOW_USER_HOME}/src \
    && chown -R airflow: ${AIRFLOW_USER_HOME}

COPY dags ${AIRFLOW_USER_HOME}/dags
COPY webserver ${AIRFLOW_USER_HOME}/webserver
